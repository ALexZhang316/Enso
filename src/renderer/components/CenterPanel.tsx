...
const CenterPanel = (props: CenterPanelProps): JSX.Element => {
  ...
  const handleSaveSettings = async (): Promise<void> => {
    ...
    try {
      ...
      withKeys.providers = { ...config.providers };
      for (const [pid, draft] of Object.entries(apiKeyDrafts)) {
        if (draft.trim()) {
          withKeys.providers[pid as ProviderId] = {
            ...withKeys.providers[pid as ProviderId],
            apiKey: draft.trim()
          };
        }
      }
      const saved = await window.enso.saveConfig(withKeys);
      ...
      <main className="flex h-full min-h-0 flex-col gap-2.5" data-testid="center-pane">
        <Card className="min-h-0 flex-1 flex flex-col overflow-hidden">
          <CardHeader className="shrink-0 flex-row items-center justify-between space-y-0">
            ...
            <Button onClick={() => void handleSaveSettings()} disabled={isLoading}>
              保存
            </Button>
          </CardHeader>
                ...
                {PROVIDER_PRESETS.map((preset) => (
                  <div key={preset.id} className="space-y-2">
                    ...
                    <label className="space-y-1">
                      ...
                      <select
                        className="h-9 w-full rounded-[10px] bg-black/[0.04] px-3 text-[13px] focus:outline-none focus:ring-2 focus:ring-primary/30"
                        value={config.providers[preset.id]?.model ?? preset.defaultModel}
                        onChange={(e) => {
                          const updated = { ...config };
                          updated.providers = { ...config.providers };
                          updated.providers[preset.id] = {
                            ...updated.providers[preset.id],
                    ...
                    <label className="space-y-1">
                      <div className="text-[12px] text-muted-foreground">
                        API Key {hasKeyStatus[preset.id] ? "（已保存）" : ""}
                      </div>
                      <Input
                        type="password"
                        value={apiKeyDrafts[preset.id] ?? ""}
                        placeholder={hasKeyStatus[preset.id] ? "••••••••" : "输入 API Key"}
                        onChange={(e) =>
                    ...
                    {hasKeyStatus[preset.id] && (
                      <Button
                        ...
                        size="sm"
                        onClick={async () => {
                          await window.enso.clearProviderApiKey(preset.id);
                          setHasKeyStatus((prev) => ({ ...prev, [preset.id]: false }));
                          setSettingsStatus(`已清除 ${preset.label} 的 API Key。`);
                        }}
                      >
                        清除 API Key
                      </Button>
        ...
        <CardContent className="p-3">
          ...
          <div className="flex items-center justify-between mt-2">
            <div className="flex items-center gap-2">
              ...
              >
                {(PROVIDER_MAP[selectedProvider]?.models ?? []).map((m) => (
                  <option key={m} value={m}>{m}</option>
                ))}
              </select>
            </div>
